...
class Mexc:
    ...
    def init_info(self, order_info: MarketOrder):
        ...
        if order_info.is_futures:
            if order_info.is_coinm:
                raise error.UnSupportedFeatureError("mexc - coinm")
            else:
    ...
    def set_leverage(self, leverage: float, symbol: str, side: str, margin_mode: str | None = None):
        try:
            openType = 2 if margin_mode == "cross" else 1   # 1: isolated, 2: cross
            positionType = 2 if side == "sell" else 1   # 1: long, 2: short
            params = {"openType": openType, "positionType": positionType}
            self.client.set_leverage(leverage, symbol, params)
        except Exception as e:
            error = str(e)
            if "Exists open orders can not change leverage" in error:
                pass
            else:
                raise Exception(e)
    ...
    def market_order(self, order_info: MarketOrder):
        raise error.UnSupportedFeatureError("mexc - market order")

    def market_buy(self, order_info: MarketOrder):
        raise error.UnSupportedFeatureError("mexc - market buy")

    def market_sell(self, order_info: MarketOrder):
        raise error.UnSupportedFeatureError("mexc - market sell")

    def market_entry(self, order_info: MarketOrder):
        raise error.UnSupportedFeatureError("mexc - market entry")

    def market_close(self, order_info: MarketOrder):
        raise error.UnSupportedFeatureError("mexc - market close")
...
##############################################################################
    ...
    def limit_order(self, order_info: MarketOrder, amount: float, price: float):   
        ...
        params = {}
        if order_info.is_futures:
            raise error.UnSupportedFeatureError("mexc 선물 거래 API 미지원")
            # if order_info.is_entry:
            #     if order_info.leverage is not None:
            #         self.set_leverage(order_info.leverage, symbol, order_info.side, order_info.margin_mode)
